...
class Item:
    def __init__(self, root):
        ...
        try:
            self.mydb = mysql.connector.connect(host='localhost', user='root', password='12345678', database='hotel_management_system')
            self.mycursor = self.mydb.cursor()

            self.w = Toplevel(root)
            self.w.title('Item')
            self.w.geometry("320x200")
            self.w.wm_iconbitmap('hotel.ico')
    ...
    def add(self):
        try:
            ...
            sql = "insert into item(item_no,item_name, item_type, item_price) values(%s,%s, %s, %s)"

            self.mycursor.execute(sql, ( self.idVar.get(),self.nameVar.get(), self.typeVar.get(),
                                        self.priceVar.get()))
